...
from PyQt5.QtWidgets import QMainWindow, QApplication, QMessageBox, QPlainTextEdit, QVBoxLayout, QWidget,\
...
class App(QMainWindow):
    ...
    def displayPrediction(self, prediction=None):
        if prediction is not None:
            mathml_output = latex2mathml.converter.convert(r"{equation}".format(equation=prediction))
            #self.textbox.setLineWrapMode(QTextEdit.LineWrapMode.NoWrap)
            self.textbox.setPlainText("{equation}".format(equation=mathml_output))
